...
description: Access a user's local or cloud Zotero library without MCP by executing the bundled JSON CLI. Use for Zotero collection/folder browsing, exact collection item retrieval, library search, paper metadata, PDF full text, annotations, notes, tags, comparisons, and literature reviews. Trigger when the user mentions Zotero, a Zotero collection/folder such as “目录下的论文”, papers in their library, PDF annotations, or adding Zotero notes/tags. Never invent library contents; ground every claim in CLI output.
---

# Zotero Research Assistant

Use the bundled `scripts/zotero_cli.py`; do not require or configure MCP. Resolve the absolute path of this skill directory before running it. Read `references/setup.md` only when installing, configuring, or troubleshooting.

## Required behavior

1. Run `python <skill-dir>/scripts/zotero_cli.py health` before the first Zotero operation in a task. If it returns `ok: false`, report the error and stop; do not answer from memory.
...
1. Resolve the collection exactly:
   ...
   `python <skill-dir>/scripts/zotero_cli.py find-collection "<name-or-path>"`
...
3. Read its bibliographic items:
   ...
   `python <skill-dir>/scripts/zotero_cli.py collection-items "<collection-key>" --recursive --limit 200`
...
`python <skill-dir>/scripts/zotero_cli.py search "<query>" --limit 50`
...
- Metadata: `python <skill-dir>/scripts/zotero_cli.py item <item-key>`
- Attachments and notes: `python <skill-dir>/scripts/zotero_cli.py children <item-key>`
- Annotations: `python <skill-dir>/scripts/zotero_cli.py annotations <item-key>`
- PDF text: `python <skill-dir>/scripts/zotero_cli.py read <item-key> --start 0 --max-chars 12000`
